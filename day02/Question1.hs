...
drawIsPossible (Draw { colour=Blue, count}) = count <= 14
drawIsPossible (Draw { colour=Green, count}) = count <= 13
drawIsPossible (Draw { colour=Red, count}) = count <= 12
...
main = do
    ...
    where 
        ...
        f game = let parsedGames = (runParser ((,) <$> gameParser <*> some roundParser) game) in
            case parsedGames of 
                Nothing -> 0
                Just ((gameId, rounds), _) -> if ((all drawIsPossible) . getMaximalDraws) rounds then gameId else 0